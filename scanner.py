...
def run_dirsearch(conn, domain_id, domain):
    report_dir = 'reports'
    ...
    if process.returncode == 0:
        # Find the newest report file for the domain
...
def main():
    ...
    for domain_record in domains:
        ...
        if not last_dirsearch_scan or last_dirsearch_scan <= datetime.now() - timedelta(hours=24):
            run_dirsearch(conn, domain_id, domain)
            subdomains = get_subdomains(api_key, domain)  # Re-fetch subdomains in case of updates
            for subdomain in subdomains:
                run_dirsearch(conn, domain_id, subdomain)
            update_domain_after_dirsearch(conn, domain_id)
            logging.info(f"Dirsearch scan completed for domain: {domain}")
